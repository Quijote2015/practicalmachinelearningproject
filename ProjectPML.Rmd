...
str(TrainData)
```
The training data set is made of 19622 observations on 160 columns. There are many columns have NA values or blank values on almost every observation. The next step is to remove them. The first seven columns give information about the people who did the test, and also timestamps so We will exlude them in our model.
```{r}
